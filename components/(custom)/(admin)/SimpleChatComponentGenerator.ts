import { SimpleChatConfig } from './SimpleChatBuilder';
...
export function generateSimpleChatComponentCode(config: SimpleChatConfig, siteName: string): { componentCode: string; validComponentName: string } {
  const validComponentName = generateValidComponentName(siteName);
  
  const quickMessagesCode = config.quickMessages.length > 0 
    ? `
            {/* Quick suggestion buttons */}
            <div className="flex gap-2 mt-3 flex-wrap">
              {[
                ${config.quickMessages.map(msg => `"${msg.replace(/"/g, '\\"')}"`).join(',\n                ')}
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      type: 'assistant',
      content: '${config.welcomeMessage.replace(/'/g, "\\'")}',
      timestamp: new Date()
    }
  ]);
